 ...
 * (medium risk)".
 *
 * @returns {string|null} null when the snapshot is missing/stale or nothing
 *                        qualifies — the Claude prompt then carries no CV line
 *                        (zero token cost).
 */
export function build() {
  const snapshot = cvWorldModel.getSnapshot();
  if (!snapshot || Date.now() - snapshot.frameTimestamp > CV_CONTEXT_STALENESS_MS) return null;

  const qualifying = snapshot.objects
